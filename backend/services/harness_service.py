...
class HarnessService:
    ...
    def create_task(self, payload: dict[str, Any]):
        ...
        task = TaskContract.create(title=str(payload.get("title", "")), goal=str(payload.get("goal", "")), risk_level=str(payload.get("risk_level", "normal")), status=str(payload.get("status", "draft")), context_snapshot_id=payload.get("context_snapshot_id"), **fields)
    ...
    def resolve_or_create_run(self, session_id: str, cwd: str):
        ...
        return self.repository.bind_session(run.run_id, session_id, key).to_dict()

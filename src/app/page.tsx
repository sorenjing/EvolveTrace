...
export default function Home() {
  ...
  useEffect(() => subscribeToAuditStream(session?.session_id ?? null, message => { setConnected(true); if (session?.session_id === message.event.session_id) setSession(current => current ? { ...current, events: [...current.events, message.event] } : current); }, () => setConnected(false)), [session?.session_id]);
